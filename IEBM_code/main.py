    ...
    "Q235B": {
        ...
        "仓库分布": {
            "上海中心仓": {
                ...
                "入库批次": [
                    {"批次号": "SH202406001", "数量(吨)": 200.0, "入库时间": "2024-06-05"},
                    {"批次号": "SH202406015", "数量(吨)": 220.5, "入库时间": "2024-06-18"}
                ]
            },
            "武汉二仓": {
                "数量(吨)": 264.8,
                "规格(mm)": ["Φ20", "板25×1500"],
                "入库批次": [
                    {"批次号": "WH202405123", "数量(吨)": 264.8, "入库时间": "2024-05-28"}
                ]
            }
        },
        "安全库存": 300.0,
    ...
    "HRB400E": {
        ...
        "仓库分布": {
            "成都抗震仓": {
                ...
                "规格(mm)": ["Φ28", "Φ32"],
                "入库批次": [
                    {"批次号": "CD202406045", "数量(吨)": 80.0, "入库时间": "2024-06-10"},
                    {"批次号": "CD202406097", "数量(吨)": 78.2, "入库时间": "2024-06-22"}
                ]
            }
        },
        "安全库存": 100.0,
    ...
    "ASTM A572-50": {
        ...
        "仓库分布": {
            "洋山保税仓": {
                ...
                "入库批次": [
                    {"批次号": "YS202406-US1", "数量(吨)": 82.6, "入库时间": "2024-06-15"}
                ]
            }
        },
        "安全库存": 50.0,
    ...
    "S30408": {
        ...
        "仓库分布": {
            "太钢特种仓": {
                ...
                "入库批次": [
                    {"批次号": "TG202406-SS1", "数量(吨)": 35.8, "入库时间": "2024-06-12"}
                ]
            }
        },
        "安全库存": 20.0,
    ...
    "6061-T6": {
        ...
        "仓库分布": {
            "西飞特供仓": {
                ...
                "入库批次": [
                    {"批次号": "XF202406-AL1", "数量(吨)": 12.5, "入库时间": "2024-06-05"}
                ]
            }
        },
        "安全库存": 5.0,
    ...
    "Q235B-XL": {
        ...
        "仓库分布": {
            "港珠澳项目仓": {
                ...
                "入库批次": [
                    {"批次号": "GZA202406-XL1", "数量(吨)": 45.0, "入库时间": "2024-06-01"}
                ]
            }
        },
        "安全库存": 10.0,
    ...
    "HRB400E-FG": {
        ...
        "仓库分布": {
            "京沪高铁材料仓": {
                ...
                "规格(mm)": ["Φ12", "Φ16", "Φ20"],
                "入库批次": [
                    {"批次号": "JHGT202406-01", "数量(吨)": 120.0, "入库时间": "2024-06-10"},
                    {"批次号": "JHGT202406-02", "数量(吨)": 100.0, "入库时间": "2024-06-25"}
                ]
            }
        },
        "安全库存": 150.0,
    ...
    "ASTM A36": {
        ...
        "仓库分布": {
            "深圳跨境仓": {
                ...
                "规格(mm)": ["W10×12", "C10×20"],
                "入库批次": [
                    {"批次号": "SZ202406-US2", "数量(吨)": 150.0, "入库时间": "2024-06-20"}
                ]
            }
        },
        "安全库存": 80.0,
...
# 钢材牌号智能检索
def steel_screen(lower_tensil_limit, higher_expense_limit):
    return [
        f"{k} ¥{v['价格(元/吨)']}"
        for k, v in steel_database.items()
        if v['抗拉强度(MPa)'] >= lower_tensil_limit
           and v['价格(元/吨)'] <= higher_expense_limit
           and v['价格(元/吨)'] > 0  # 排除测试数据
    ]
...
class inventory_manager:
    ...
    def inventory_Warning(self):
        return [k for k, v in steel_database.items()
...
# 身份：工人
    ...
    # 多功能获取
    function = input('请输入功能【Ａ】库存查询【B】库存更新 【C】库存检查 【D】物料调度')
    # 功能：库存查询
    if function == 'A':
        original_data = input('请输入查询信息：{钢材牌号}{查询仓库}')
        trackmark_input, stash_input = original_data.split()
        # 查询库存并打印
        print(inventory_manager.inventory_check(trackmark_input, stash_input))
    # 功能：库存更新
    elif function == 'B':
        pass
    # 功能：库存检查
    ...
    # 功能：物料调度
        ...
        trackmark_input, type_input, quantity_input = original_data.split()
        quantity_input = int(quantity_input)
        # 生成库存分布列表
        inventory_recommence = [
            stash_name for stash_name,stash_value in steel_database[trackmark_input]["库存数据"]["仓库分布"]
